...
# --------------------------------------------------------------------------
# run_migrate_doc_types
# --------------------------------------------------------------------------


def test_migrate_doc_types_store_rows_migrated_with_counts(tmp_path, monkeypatch):
    store = make_store(tmp_path)
    store.ensure_tables()
    patch_store_factory(monkeypatch, store)

    for i, doc_type in enumerate(["surat", "surat", "other", "mom"]):
        store.insert_document(
            {
                "doc_id": f"doc{i:04d}",
                "file_name": f"{i}.pdf",
    ...
    report = pipeline.run_migrate_doc_types([])

    assert any("store: 2 document(s) surat -> letter" in w for w in report.warnings)
    assert any("store: 1 document(s) other -> others" in w for w in report.warnings)
    ...
    assert counts == {"letter": 2, "others": 1, "mom": 1}
    store.close()


def test_migrate_doc_types_store_dry_run_writes_nothing(tmp_path, monkeypatch):
    store = make_store(tmp_path)
    store.ensure_tables()
    patch_store_factory(monkeypatch, store)
    ...
    report = pipeline.run_migrate_doc_types([], dry_run=True)

    assert any("store: 1 document(s) surat -> letter" in w for w in report.warnings)
    doc = store.get_document("doc0000")
    assert doc["doc_type"] == "surat"  # untouched
    store.close()


def test_migrate_doc_types_sidecar_rewritten_only_doc_type_changed(
    tmp_path, monkeypatch
):
    store = make_store(tmp_path)
    store.ensure_tables()
    ...
    other_sc = make_sidecar(
        tmp_path, "other.pdf", reviewed=True, file_hash="bb" * 32, doc_type="other"
    )
    current_sc = make_sidecar(
        tmp_path, "current.pdf", reviewed=True, file_hash="cc" * 32, doc_type="mom"
    )

    report = pipeline.run_migrate_doc_types([tmp_path])

    legacy_meta, _ = read_sidecar(legacy_sc)
    assert legacy_meta["doc_type"] == "letter"
    assert legacy_meta["subject"] == "original subject"  # other keys untouched
    assert legacy_meta["reviewed"] is False  # reviewed flag untouched

    other_meta, _ = read_sidecar(other_sc)
    assert other_meta["doc_type"] == "others"
    assert other_meta["reviewed"] is True  # reviewed flag untouched

    current_meta, _ = read_sidecar(current_sc)
    assert current_meta["doc_type"] == "mom"  # not a legacy value, untouched

    assert set(report.processed) == {"legacy.corpus.md", "other.corpus.md"}
    assert report.skipped == ["current.corpus.md"]
    store.close()


def test_migrate_doc_types_sidecar_dry_run_writes_nothing(tmp_path, monkeypatch):
    store = make_store(tmp_path)
    store.ensure_tables()
    patch_store_factory(monkeypatch, store)

    sc = make_sidecar(
        tmp_path, "legacy.pdf", reviewed=False, file_hash="aa" * 32, doc_type="surat"
    )

    report = pipeline.run_migrate_doc_types([tmp_path], dry_run=True)

    meta, _ = read_sidecar(sc)
    assert meta["doc_type"] == "surat"  # untouched
    assert report.processed == ["legacy.corpus.md"]
    store.close()


def test_migrate_doc_types_bad_sidecar_isolated(tmp_path, monkeypatch):
    store = make_store(tmp_path)
    store.ensure_tables()
    patch_store_factory(monkeypatch, store)
    ...
    make_sidecar(
        tmp_path, "good.pdf", reviewed=True, file_hash="11" * 32, doc_type="surat"
    )
